...
namespace Fluent {
	...
	public partial class OLVColumn : ColumnHeader {
		...
		public bool AutoCompleteEditor {
			get => AutoCompleteEditorMode != AutoCompleteMode.None;
			set {
				if (value) {
		...
		public virtual bool CellEditUseWholeCellEffective {
			get {
				var columnSpecificValue = ListView.View == View.Details ? CellEditUseWholeCell : (bool?) null;
				return columnSpecificValue ?? ((AdvancedListView) ListView).CellEditUseWholeCell;
			}
		}
		...
		public bool EnableButtonWhenItemIsDisabled {
			get => enableButtonWhenItemIsDisabled;
		...
		public string GroupWithItemCountFormatOrDefault {
			get {
				...
				if (ListView != null) {
					cachedGroupWithItemCountFormat = ((AdvancedListView) ListView).GroupWithItemCountFormatOrDefault;
		...
		public string GroupWithItemCountSingularFormatOrDefault {
			get {
				...
				if (ListView != null) {
					cachedGroupWithItemCountSingularFormat = ((AdvancedListView) ListView).GroupWithItemCountSingularFormatOrDefault;
		...
		public Color HeaderForeColor {
			get => HeaderFormatStyle == null ? Color.Empty : HeaderFormatStyle.Normal.ForeColor;
			set {
		...
		public bool HeaderCheckBoxUpdatesRowCheckBoxes {
			get => headerCheckBoxUpdatesRowCheckBoxes;
		...
		public bool IsButton {
			...
			set {
				isButton = value;
				if (value) {
					var buttonRenderer = Renderer as ColumnButtonRenderer;
		...
		public RenderDelegate RendererDelegate {
			get {
				var version1Renderer = Renderer as Version1Renderer;
		...
		public string ConvertGroupKeyToTitle(object value) {
			...
			return value == null ? AdvancedListView.GroupTitleDefault : ValueToString(value);
		}
		...
		public string[] GetSearchValues(object rowObject) {
			...
			var dtr = Renderer as DescribedTaskRenderer;
			if (dtr != null) {
		...
		public Type DataType {
			get {
				if (dataType == null) {
					var olv = ListView as AdvancedListView;
					if (olv != null) {
		...
		public void MakeGroupies<T>(T[] values, string[] descriptions, object[] images, string[] subtitles, string[] tasks) {
			...
			GroupFormatter = delegate(OLVGroup group, GroupingParameters parms) {
				var key = (int) group.Key; // we know this is an int since we created it in GroupKeyGetter
		...
		public void MakeEqualGroupies<T>(T[] values, string[] descriptions, object[] images, string[] subtitles, string[] tasks) {
			...
			// Install a delegate that returns the index of the description to be shown
			GroupKeyGetter = delegate(object row) { return valuesArray.IndexOf(GetValue(row)); };

			// Install a delegate that simply looks up the given index in the descriptions.
			GroupKeyToTitleConverter = delegate(object key) {
				var intKey = (int) key; // we know this is an int since we created it in GroupKeyGetter
